...
// cb_predictions: A uint64_t packed with "m" taken/not-taken conditional branch predictions.
   ...
   while ((pos < banks) && !terminated) {	// "pos" is position of the instruction within the maximum-length sequential fetch bundle.
	 ...
	 switch (bundle[pos].branch_type) {
	    case BTB_BRANCH:
	       ...
	       // The least significant bit of cb_predictions is the next taken/not-taken prediction (because we shift it right, subsequently).
	       // From this bit, set the taken flag, accordingly.
	       taken = ((cb_predictions & 1) == 1);

	       // Shift out the used-up prediction bit, to set up prediction of the next conditional branch.
	       cb_predictions = (cb_predictions >> 1);
