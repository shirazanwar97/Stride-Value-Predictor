		...
		switch (inst.opcode()) {
			...
			case OP_STORE:
				if (!SPLIT_STORES) {
				   ...
				   // Initialize the lower uop.
				   split(index);

				   // store-addr uop
				   PAY.buf[index].split = true;
				   PAY.buf[index].split_store = true;
				   PAY.buf[index].upper = true;
				   // base register for AGEN
				   PAY.buf[index].A_valid = true;
				   PAY.buf[index].A_log_reg = inst.rs1();
				   
				   // store-val uop
				   PAY.buf[index+1].split = true;
				   PAY.buf[index+1].split_store = true;
				   PAY.buf[index+1].upper = false;
				   // source register
				   PAY.buf[index+1].A_valid = true;
				   PAY.buf[index+1].A_log_reg = inst.rs2();
				}
				break;
			...
			case OP_STORE_FP:
				if (!SPLIT_STORES) {
				   ...
				   // source register
				   PAY.buf[index+1].A_valid = true;
				   PAY.buf[index+1].A_log_reg = inst.rs2()+NXPR;
				}
				break;
...
void pipeline_t::split(uint64_t index) {
   PAY.buf[index+1].inst             = PAY.buf[index].inst;
   PAY.buf[index+1].pc               = PAY.buf[index].pc;
   PAY.buf[index+1].next_pc          = PAY.buf[index].next_pc;
   PAY.buf[index+1].branch           = PAY.buf[index].branch;
   PAY.buf[index+1].branch_type      = PAY.buf[index].branch_type;
   PAY.buf[index+1].branch_target    = PAY.buf[index].branch_target;
   PAY.buf[index+1].good_instruction = PAY.buf[index].good_instruction;
   PAY.buf[index+1].db_index         = PAY.buf[index].db_index;
   PAY.buf[index+1].sequence         = PAY.buf[index].sequence;
   PAY.buf[index+1].pred_tag         = PAY.buf[index].pred_tag;
   PAY.buf[index+1].flags            = PAY.buf[index].flags;
   PAY.buf[index+1].fu               = PAY.buf[index].fu;
   PAY.buf[index+1].latency          = PAY.buf[index].latency;
   PAY.buf[index+1].checkpoint       = PAY.buf[index].checkpoint;
   // split
   // upper
   // split_store
   PAY.buf[index+1].A_valid          = false;
   // A_log_reg
   PAY.buf[index+1].B_valid          = false;
   // B_log_reg
   PAY.buf[index+1].C_valid          = false;
   // C_log_reg
   PAY.buf[index+1].D_valid          = false;
   // D_log_reg
   PAY.buf[index+1].iq               = PAY.buf[index].iq;
   // CSR_addr
   // size
   // is_signed
   // left
   // right
   assert(PAY.buf[index].fflags == 0);
   PAY.buf[index+1].fflags           = PAY.buf[index].fflags;
   assert(!PAY.buf[index].trap.valid());   // We shouldn't be splitting an instruction that suffered a fetch exception.
   PAY.buf[index+1].trap.clear();
   assert(!PAY.buf[index+1].trap.valid());
}
