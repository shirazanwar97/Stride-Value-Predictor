...
void pipeline_t::dispatch() {
   ...
   bool D_ready;
   db_t* actual;
   db_t* ValPredactual; //mona.lisa2 for task2 p4
   ...
   // FIX_ME #6 BEGIN
      if(REN->stall_dispatch(i)) //i or dispatch_width?
         return;
   // FIX_ME #6 END
      ...
      // FIX_ME #9 BEGIN
         ...
         if(PAY.buf[index].C_valid){
            ...
            if(PAY.buf[index].isVPEligible) {
               if (PERFECT_VALUE_PRED && PAY.buf[index].good_instruction) { //perfect Value prediction
                  ValPredactual = get_pipe()->peek(PAY.buf[index].db_index);
                  REN->set_ready(PAY.buf[index].C_phys_reg);
                  REN->write(PAY.buf[index].C_phys_reg,  ValPredactual->a_rdst[0].value);
                  PAY.buf[index].isConfident = true;
                  PAY.buf[index].isCorrect = true;
               }  //mona.lisa2 task 1 p4
               //STRIDE VALUE PREDICTION CODE
               else if((!PERFECT_VALUE_PRED) && PAY.buf[index].good_instruction && oracleConf) {  //oracle mode
                  ValPredactual = get_pipe()->peek(PAY.buf[index].db_index);
                  if(PAY.buf[index].isVPValid) {  //prediction happened
                     if((ValPredactual->a_rdst[0].value) == (PAY.buf[index].predictedValue)) { //predicted value is correct
                        REN->set_ready(PAY.buf[index].C_phys_reg);
                        PAY.buf[index].isConfident = true;
                        PAY.buf[index].isCorrect = true;
                        REN->write(PAY.buf[index].C_phys_reg,  PAY.buf[index].predictedValue);
                     }
                     else {  //unconf incorr case
                        PAY.buf[index].isConfident = false;
                        PAY.buf[index].isCorrect = false;
                     }                        
                  } 
               } else if((!PERFECT_VALUE_PRED) && !(oracleConf)){ //real mode
                  if(PAY.buf[index].isConfident) {  //prediction happened
                     REN->set_ready(PAY.buf[index].C_phys_reg);
                     REN->write(PAY.buf[index].C_phys_reg,  PAY.buf[index].predictedValue);
                  }
               }
