...
void pipeline_t::execute(unsigned int lane_number) {
      ...
      if (IS_MEM_OP(PAY.buf[index].flags)) {
	    ...
	    // check the existence (validity) of a destination register.
            ...
            if(hit && (PAY.buf[index].C_valid)){
               ...
               //write actual value to VPQ's value - this is just for task 3,4
               if(PAY.buf[index].isVPEligible && (!PERFECT_VALUE_PRED) ) {
                  VP->vpq.setVPQValue(PAY.buf[index].pc, PAY.buf[index].C_value.dw, PAY.buf[index].VPQTail);
               }

               if(PAY.buf[index].C_valid && (!PERFECT_VALUE_PRED) && (!(oracleConf))){
                  if( PAY.buf[index].isConfident) {
                     if(PAY.buf[index].C_value.dw != PAY.buf[index].predictedValue){
   ...
   if (depth > 0) {
	 ...
	 //
	   ...
	   // FIX_ME #11b BEGIN
         bool hasDest = PAY.buf[index].C_valid;
         bool isLoad = IS_LOAD(PAY.buf[index].flags);
         bool isAMO = IS_AMO(PAY.buf[index].flags);
         if(hasDest && (isLoad == false) && (isAMO == false)){
            IQ.wakeup(PAY.buf[index].C_phys_reg);
            REN->set_ready(PAY.buf[index].C_phys_reg);
         }
      // FIX_ME #11b END
