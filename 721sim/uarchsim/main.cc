...
static void set_perfect_flags(const char* config) {
   ...
   if (sscanf(config, "%lu,%lu,%lu,%lu", &pbp, &pdc, &pic, &ptc) != 4) {
      fprintf(stderr, "Incorrect usage of --perf=<pbp>,<pdc>,<pic>,<ptc>,<pvp>\n");
      fprintf(stderr, "...where pbp (perfect branch prediction), pdc (perfect D$), pic (perfect I$), ptc (perfect T$)) are each 0 or 1.\n");
      exit(-1);
   }
   else {
...
//mona.lisa2 for task6 p4 perfect value prediction
static void set_perfect_value_prediction(const char* config) {
   uint64_t pvp;
   if (sscanf(config, "%lu", &pvp) != 1) {
      fprintf(stderr, "Incorrect usage of --perf=<pvp>\n");
      fprintf(stderr, "...where pvp (perfect value prediction)) are each 0 or 1.\n");
      exit(-1);
   }
   else {
      PERFECT_VALUE_PRED = (pvp ? true : false);
      if(VALUE_PRED_EN == 0 && PERFECT_VALUE_PRED == 1) {
         fprintf(stderr, "Incorrect usage of perfect value prediction\n");
         fprintf(stderr, "...where perfect value prediction is 1 and value prediction enable is 0.)\n");
         exit(-1);
      }
   }
}

//mona.lisa2 for task6 p4 stride value prediction
/*
...
*/
static void set_stride_value_prediction(const char* config) {
uint64_t cliVPQsize;  
uint64_t clioracleConf;  
uint64_t cliSVPIndexBits;  
uint64_t cliSVPTagBits;  
uint64_t cliconfMax;  
uint64_t cliconfInc;  
uint64_t cliconfDec;  
uint64_t clireplace_stride;  
uint64_t clireplace;  
uint64_t clipredINTALU;  
uint64_t clipredFPALU;  
uint64_t clipredLOAD;  
uint64_t cliVPQ_full_policy; 
   if (sscanf(config, "%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu", &cliVPQsize, 
              &clioracleConf, &cliSVPIndexBits, &cliSVPTagBits, &cliconfMax, &cliconfInc, &cliconfDec,
              &clireplace_stride, &clireplace, &clipredINTALU, &clipredFPALU, &clipredLOAD, &cliVPQ_full_policy) != 13) {
      fprintf(stderr, "Incorrect usage of oracleConf, SVPIndexBits, SVPTagBits, confMax, confInc, confDec, replace_stride, replace, predINTALU, predFPALU, predLOAD, VPQ_full_policy \n");
      exit(-1);
   }
   else {
      VPQsize   = cliVPQsize;  
      oracleConf   = (clioracleConf ? 1 : 0);  
      SVPIndexBits   = cliSVPIndexBits;  
      SVPTagBits   = cliSVPTagBits;  
      confMax   = cliconfMax;  
      confInc   = cliconfInc;  
      confDec   = cliconfDec;  
      replace_stride   = clireplace_stride;  
      Replace   = clireplace;  
      predINTALU   = (clipredINTALU ? 1 : 0);  
      predFPALU   =  (clipredFPALU  ? 1 : 0);  
      predLOAD   =   (clipredLOAD   ? 1 : 0);  
      VPQ_full_policy = (cliVPQ_full_policy ? 1: 0); 
      //if(VALUE_PRED_EN == 0 && PERFECT_VALUE_PRED == 1) {
      //   fprintf(stderr, "Incorrect usage of perfect value prediction\n");
      //   fprintf(stderr, "...where perfect value prediction is 1 and value prediction enable is 0.)\n");
      //   exit(-1);
...
int main(int argc, char** argv)
{
  ...
  parser.option('L', 0, 0, [&](const char* s){
      ...
      NUM_CHECKPOINTS = 64;
      FETCH_QUEUE_SIZE = 64;});
   parser.option(0, "vp-enable", 1, [&](const char* s){VALUE_PRED_EN = atoi(s);});  //mona.lisa2 added pvp for task6 p4
   parser.option(0, "vp-perf", 1, [&](const char* s){set_perfect_value_prediction(s);});  //mona.lisa2 added pvp for task6 p4
   parser.option(0, "vp-svp", 1, [&](const char* s){set_stride_value_prediction(s);});  //mona.lisa2 added pvp for task6 p4
