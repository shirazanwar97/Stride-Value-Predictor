  ...
  LSU(lq_size, sq_size, Tid, _mmu, this)
{
  ...
  fprintf(stats_log, "   VPQsize         = %d\n", VPQsize);
  fprintf(stats_log, "   oracleconf      = %d (oracle confidence)\n", (oracleConf ? 1 : 0));
  fprintf(stats_log, "   # index bits    = %d\n", SVPIndexBits);
  fprintf(stats_log, "   # tag bits      = %d\n", SVPTagBits);
  fprintf(stats_log, "   confmax         = %d\n", confMax);
  fprintf(stats_log, "   confinc         = %d\n", confInc);
  fprintf(stats_log, "   confdec         = %d (reset)\n", confDec);
  fprintf(stats_log, "   replace_stride  = %d\n", replace_stride);
  fprintf(stats_log, "   replace         = %d\n", Replace);
  fprintf(stats_log, "   predINTALU      = %d\n", predINTALU);
  fprintf(stats_log, "   predFPALU       = %d\n", predFPALU);
  fprintf(stats_log, "   predLOAD        = %d\n", predLOAD);
  fprintf(stats_log, "   VPQ_full_policy = %d (stall bundle)\n", VPQ_full_policy);
  
  uint64_t confBits = (uint64_t)ceil(log2((double)(confMax+1)));
  uint64_t instCtr =  (uint64_t)ceil(log2((double)VPQsize));
  uint64_t bitsPerSVPEntry = (SVPTagBits + confBits + 128 + instCtr);
  uint64_t bitsPerVPQEntry = (64+ SVPTagBits+ SVPIndexBits);
  uint64_t SVPTotalSize = (pow(2,SVPIndexBits));
  uint64_t SVPTotalbits = (SVPTotalSize*bitsPerSVPEntry);
  uint64_t VPQTotalbits = (bitsPerVPQEntry*VPQsize);
  uint64_t storageBits =  SVPTotalbits+ VPQTotalbits;
  double storageBytes = storageBits / 8.0; 
  double storageKB = storageBytes / 1024.0; 
  fprintf(stats_log, "COST ACCOUNTING\n");
  fprintf(stats_log, "   One SVP entry:\n");
  fprintf(stats_log, "      tag           :  %lu bits  // num_tag_bits\n", SVPTagBits);
  fprintf(stats_log, "      conf          :   %lu bits  // formula: (uint64_t)ceil(log2((double)(confmax+1)))\n", confBits);
  fprintf(stats_log, "      retired_value :  %lu bits  // RISCV64 integer size.\n", 64);
  fprintf(stats_log, "      stride        :  %lu bits  // RISCV64 integer size. Competition opportunity: truncate stride to far fewer bits based on stride distribution of stride-predictable instructions.\n", 64);
  ...
  fprintf(stats_log, "      PC_tag        :  %lu bits  // num_tag_bits\n", SVPTagBits);
  fprintf(stats_log, "      PC_index      :  %lu bits  // num_index_bits\n", SVPIndexBits);
  fprintf(stats_log, "      value         :  %lu bits  // RISCV64 integer size.\n", 64);
  fprintf(stats_log, "      -------------------------\n");
  fprintf(stats_log, "      bits/VPQ entry: %lu bits\n", (64+ SVPTagBits+ SVPIndexBits));
  fprintf(stats_log, "   Total storage cost (bits) = %lu (%lu SVP entries x %lu bits/SVP entry) + %lu (%lu VPQ entries x %lu bits/VPQ entry) = %lu bits\n", SVPTotalbits, SVPTotalSize, bitsPerSVPEntry, VPQTotalbits, VPQsize, bitsPerVPQEntry, storageBits);
  fprintf(stats_log, "   Total storage cost (bytes) = %.2f B (%.2f KB)\n", storageBytes, storageKB);
  }
...
pipeline_t::~pipeline_t()
{
  ...
  fprintf(stats_log, "vpmeas_ineligible         :    %ld \n", vpmeas_ineligible);     	// Not eligible for value prediction.
  fprintf(stats_log, "   vpmeas_ineligible_type :    %ld \n", vpmeas_ineligible_type);     	// Not eligible for value prediction.
  fprintf(stats_log, "   vpmeas_ineligible_drop :    %ld \n\n", vpmeas_ineligible_drop);     	// Not eligible for value prediction.
  fprintf(stats_log, "vpmeas_eligible           :    %ld \n", vpmeas_eligible);     	// Eligible for value prediction.
  fprintf(stats_log, "   vpmeas_miss            :    %ld \n", vpmeas_miss);     // VPU was unable to generate a value prediction (e.g., SVP miss).
  fprintf(stats_log, "   vpmeas_conf_corr       :    %ld \n", vpmeas_conf_corr);     	// VPU generated a confident and correct value prediction.
  fprintf(stats_log, "   vpmeas_conf_incorr     :    %ld \n", vpmeas_conf_incorr);     // VPU generated a confident and incorrect value prediction. (MISPREDICTION)
  fprintf(stats_log, "   vpmeas_unconf_corr     :    %ld \n", vpmeas_unconf_corr);     // VPU generated an unconfident and correct value prediction. (LOST OPPORTUNITY)
  fprintf(stats_log, "   vpmeas_unconf_incorr   :    %ld \n", vpmeas_unconf_incorr);     	// VPU generated an unconfident and incorrect value prediction.
    ...
    if (unlikely(debug))
    {
      ...
      while (instret < instret_limit)
      {
        ...
        for (lane_number = 0; lane_number < RETIRE_WIDTH; lane_number++) {
          ...
          // if(((num_insn % 1000) == 0) && (num_insn!=0) ){
            // cout << "-------- DEBUG: retired instruction count =  " << std::dec << num_insn << "++++++++++++++++++++++++" << endl;
            // VP->printSVP();
            // VP->printVPQ();
          // }
