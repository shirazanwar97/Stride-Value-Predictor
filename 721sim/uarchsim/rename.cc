...
void pipeline_t::rename2() {
   ...
   unsigned int checkpointNeededInstructionCount = 0;
   unsigned int instructionWithDestRegCount = 0;
   unsigned int vpEligibleInstructionCount = 0;
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      PAY.buf[index].isVPEligible = false;  //mona.lisa2 for task 3
   	PAY.buf[index].isVPValid = false;
   	PAY.buf[index].VPQTail = 0;
   	PAY.buf[index].VPQTailPhase = false;
   	PAY.buf[index].predictedValue = 0;
   	PAY.buf[index].isSVPMiss = false;
      PAY.buf[index].isConfident = false;
      PAY.buf[index].isCorrect = false;
      PAY.buf[index].isDroppedInstruction = false;
   	PAY.buf[index].isoriginallyIneligible = false;

      // FIX_ME #1 BEGIN
      if(PAY.buf[index].checkpoint)
         checkpointNeededInstructionCount++;
      if(PAY.buf[index].C_valid)
         instructionWithDestRegCount++;
      if( PAY.buf[index].C_valid && VP->isEligible(PAY.buf[index].flags) && (!(PAY.buf[index].checkpoint)) && VALUE_PRED_EN){ //check for !PERFVP in below rename logic
            PAY.buf[index].isVPEligible = true;
            vpEligibleInstructionCount++;
      } else {
         PAY.buf[index].isVPEligible = false;
         PAY.buf[index].isoriginallyIneligible = true;
      }
      // FIX_ME #1 END
   }
   ...
   // FIX_ME #2 BEGIN
      if((REN->stall_reg(instructionWithDestRegCount) || REN->stall_branch(checkpointNeededInstructionCount) || VP->vpq.stall_value(vpEligibleInstructionCount))) //1.
         return;
   // FIX_ME #2 END
   
   if((VPQ_full_policy)){
      vpEligibleInstructionCount = VP->vpq.getAvailableEntriesInVPQ();
      for(unsigned int i=0; i < dispatch_width; i++){
         index = RENAME2[i].index;
         if(PAY.buf[index].isVPEligible){
            if(vpEligibleInstructionCount == 0){
               PAY.buf[index].isVPEligible = false;
               PAY.buf[index].isDroppedInstruction = true;
            }
            else 
               vpEligibleInstructionCount--;
         }
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      if(PAY.buf[index].checkpoint){
         ...
         PAY.buf[index].VPQTail = VP->vpq.getVPQTail();
         PAY.buf[index].VPQTailPhase = VP->vpq.getVPQTailPhase();
      }

      if(PAY.buf[index].isVPEligible && (!PERFECT_VALUE_PRED)){ //should be else if?--> no, otherwise set isVPValdid=false & predictedValue=
         assert(PAY.buf[index].isVPEligible && "not a vp eligible instruction but is writing to vpq in renamer");
         assert((!PAY.buf[index].checkpoint) && "you got a checkpointed/branch Instrcution which is eligible??");
         uint64_t prediction = 0; //UINT64_MAX
         bool confidence = false;
         PAY.buf[index].isSVPMiss = (!VP->handleSVPEntry(PAY.buf[index].pc));
         PAY.buf[index].VPQTail = VP->vpq.setVPQTag(PAY.buf[index].pc); //checkpointing VPQTail
         if(PAY.buf[index].isSVPMiss){
            PAY.buf[index].isVPValid = false;
         }
         else{
            PAY.buf[index].isVPValid = true;
            VP->predValue(prediction, confidence, PAY.buf[index].pc);
            PAY.buf[index].predictedValue = prediction;
            PAY.buf[index].isConfident = confidence;
         }
      } //can write and else for all to set isVPValid=false and predictedValue=0;
      // FIX_ME #5 END
   }
