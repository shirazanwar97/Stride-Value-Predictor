...
private:
	...
	uint64_t logicalRegSize; 
	uint64_t activeListSize;
	uint64_t noOfCheckpoint;
	/////////////////////////////////////////////////////////////////////
	...
	/////////////////////////////////////////////////////////////////////
    struct free_list{
        uint64_t freeList_Head, freeList_Tail;
        bool freeListHeadPhase, freeListTailPhase;
        vector<uint64_t> freeListEntry;
    };
	...
	struct activeListEntry {
		bool destFlag; // destination flag (indicates whether or not the instr. has a destination register)
		uint64_t logicalRegDest, physicalRegDest; // logical register number, physical register number of the instruction's destination
		bool isCompleted; 	//	completion status
		bool isException; 	//	exception status
		bool isLoadVoilated; 	//	what the fuck is this?
		bool isBranchMispredicted; 	//	branch misprediction bit
		bool isValueMispredicted; 	//	value misprediction bit
		bool isLoadInstr, isStoreInstr, isBranchInstr, isAtomicMemoryOp, isSystemInstr;
		uint64_t pc;
	};

    struct active_list{
        uint64_t activeList_Head, activeList_Tail;
        uint64_t activeListHeadPhase, activeListTailPhase;
        vector<activeListEntry> activeListTable;
    };
    ...
    struct branchCheckpointEntry{
        uint64_t freeListHeadCheckpoint;
        uint64_t freeListHeadPhaseCheckpoint;
        uint64_t Checkpointed_GBM;
        vector<uint64_t> shadowMapTable;
        bool valid;
    };

    vector<branchCheckpointEntry> branchCheckpointTable;

	/////////////////////////////////////////////////////////////////////
	// Private functions.
	// e.g., a generic function to copy state from one map to another.
	/////////////////////////////////////////////////////////////////////
	void initialiseFreeListAsFull();
	void initialiseActiveListAsEmpty();
	void initialiseActiveListValues();
	uint64_t getAvailableFreePhysicalReg();
	uint64_t countZeroBits();
	void advanceFreeListHead();
	void advanceFreeListTail();
	void advanceActiveListHead();
	void advanceActiveListTail();
	uint64_t findFirstzeroBit();
	uint64_t getAvailableActiveListEntry();
