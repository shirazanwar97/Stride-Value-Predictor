...
// =======================================================skhan25==========================================================================
      //if committted instruction is eligible for value prediction, train the VPQ
      if(PAY.buf[PAY.head].isVPEligible && (!PERFECT_VALUE_PRED)) { //how to get index here?
         assert((PAY.buf[PAY.head].VPQTail == VP->vpq.getVPQHead()) && "the vpqtail that was checkpointed while creating vpq entry for this instruction does not match with the vpq head.");
         vector<vpQueueEntry> vpqTable = VP->vpq.getVPQTable();
         uint64_t value = vpqTable[VP->vpq.getVPQHead()].value;
         uint64_t addr = PAY.buf[PAY.head].pc;
         VP->train(addr, value);
      }

      if(VALUE_PRED_EN /*&& (!(PERFECT_VALUE_PRED))*/){ //counter work
         if((PAY.buf[PAY.head].isVPEligible)){
            vpmeas_eligible++;
            if(PERFECT_VALUE_PRED) { //perfect val
               if(PAY.buf[PAY.head].isSVPMiss)
                  vpmeas_miss++;
               if(PAY.buf[PAY.head].isConfident && PAY.buf[PAY.head].isCorrect)  //prediction happened
                  vpmeas_conf_corr++;
               else if(PAY.buf[PAY.head].isConfident && (!PAY.buf[PAY.head].isCorrect))
                  vpmeas_conf_incorr++;
               else if((!PAY.buf[PAY.head].isConfident) && (PAY.buf[PAY.head].isCorrect))
                  vpmeas_unconf_corr++;
               else if(!(PAY.buf[PAY.head].isConfident) && (!PAY.buf[PAY.head].isCorrect))
            ...
            else { //oracle and real case
               if(PAY.buf[PAY.head].isSVPMiss)
                  vpmeas_miss++;
               else if(PAY.buf[PAY.head].isVPValid){
                  if(PAY.buf[PAY.head].isConfident && PAY.buf[PAY.head].isCorrect)  //prediction happened
                     vpmeas_conf_corr++;
                  else if(PAY.buf[PAY.head].isConfident && (!PAY.buf[PAY.head].isCorrect))
                     vpmeas_conf_incorr++;
         ...
         if((!(PAY.buf[PAY.head].isVPEligible))){
            vpmeas_ineligible++;         
            if(PAY.buf[PAY.head].isoriginallyIneligible)
               vpmeas_ineligible_type++;
            else if(PAY.buf[PAY.head].isDroppedInstruction)
